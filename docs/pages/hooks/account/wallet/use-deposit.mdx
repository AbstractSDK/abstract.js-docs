...
- `assets`: An array of `Asset` objects to deposit. An `Asset` can be either a `NativeAsset` or a `CW20Asset`:

For `NativeAsset`:

- `type`: The type of asset, which is always `'native'`
...
- `type`: The type of asset, which is always `'cw20'`
